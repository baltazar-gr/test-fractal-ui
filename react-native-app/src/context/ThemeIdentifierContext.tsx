...
import {usePlatformColorScheme} from '@bma98/fractal-ui';
...
export type ThemeIdentifierContextType = [
  ThemeIdentifierType,
  Dispatch<SetStateAction<ThemeIdentifierType>>,
];

const defaultValue: ThemeIdentifierContextType = ['light', placeholderFunction];

export const ThemeIdentifierContext = createContext<ThemeIdentifierContextType>(
  defaultValue,
);
...
export function ThemeIdentifierProvider({
  children,
  handleThemeManually,
}: ThemeIdentifierProviderProps): JSX.Element {
  ...
  return (
    <ThemeIdentifierContext.Provider value={handleState}>
      {children}
    </ThemeIdentifierContext.Provider>
  );
}
